...
const PerksPage = (user: any) => {
    ...
    const[modalTitle, setModalTitle] = useState("Create Room Perks");
    const[submitTitle, setSubmitTitle] = useState("Create a Perk");
  
    useEffect(() => {
        dispatch({type:"SET", key:'context', payload:'[roomperkspage]'});
    }, [])
    ...
    const fetchPerks = useCallback(() => {
      let _url = "/room-perks/get";
    ...
    useEffect(() => {
      ...
      if(state?.updaterecord){
          ...
          makeRequest({url:data_url, method:'get', data:null}).then(([status, response])=> {
              ...
              setModalTitle('Update Perk Details');
              setShowModal(true);
          })
      }
  },[state?.updaterecord])
